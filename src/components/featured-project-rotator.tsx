...
export function FeaturedProjectRotator({ locale, projects }: FeaturedProjectRotatorProps) {
  ...
  return (
    <div className="relative aspect-[4/3] min-h-0 overflow-hidden bg-line shadow-soft">
      ...
      <Link
        aria-label={locale === "en" ? `Open case ${activeProject.title}` : `Открыть кейс ${activeProject.title}`}
      ...
      {hasMultipleProjects ? (
        <>
          <button
            aria-label={locale === "en" ? "Previous case" : "Предыдущий кейс"}
          ...
          <button
            aria-label={locale === "en" ? "Next case" : "Следующий кейс"}
          ...
          <div className="absolute right-4 top-4 z-30 flex gap-2">
            {projects.map((project, index) => (
              <button
                aria-label={locale === "en" ? `Show case ${project.title}` : `Показать кейс ${project.title}`}
